...
import starvationevasion.client.GUI.DraftLayout.map.*;
...
public class GamePhaseMapController extends MapController
{
  ...
  public void handleMouseClicked(MouseEvent event)
  {
    ...
    if (region.getEffect() instanceof InnerShadow)
    {
      ...
      {
        starvationevasion.client.GUI.DraftLayout.map.Map.currentlySelectedRegion = Optional.empty();
        region.setEffect(unselectEffect);
      }
      else
      {
        starvationevasion.client.GUI.DraftLayout.map.Map.currentlySelectedRegion = Optional.of(MapUtil.regionIdToEnumRegion(region.getId()));
