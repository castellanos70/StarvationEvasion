...
public class ConcurrentCommModule2 implements Communication
{
   ...
   */
  public ConcurrentCommModule2(String host, int port)
  {
    HOST = host;
    PORT = port;
  ...
  public void connect()
  {
    boolean checkConnect = IS_CONNECTED.getAndSet(true);
    if (checkConnect) return; // Connect was already called
  ...
  public void dispose()
  {
    boolean checkConnect = IS_CONNECTED.getAndSet(false);
    if (!checkConnect) return; // Already disposed/the connection never succeeded
    try
    {
